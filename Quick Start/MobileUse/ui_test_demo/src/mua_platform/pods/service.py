...
class PodPoolService:
    ...
    async def refresh(
        ...
        *,
        ...
        async with _refresh_lock(config.product_id):
        ...
        *,
        ...
        try:
            if before_sync is not None:
                before_sync()
            self.repository.sync(config.product_id, page, seen_at=seen_at)
    ...
    def list_cached(self, config: RunnerConfig) -> PodPoolSnapshot:
        _validate_config(config)
        return self.repository.list_snapshot(config.product_id, now=self.clock.now())
    ...
    async def allocate_for_case(
        ...
        *,
        ...
        failure.product_id = config.product_id
        ...
        snapshot = self.repository.list_snapshot(config.product_id, now=now)
        cache_age = (
        ...
        if preferred_pod_id:
            ...
            if not preferred:
                all_pods = self.repository.get(config.product_id, preferred_pod_id)
                if all_pods is None:
...
def _base_execution_snapshot(
    ...
    if not config.tos_bucket or not config.tos_region:
