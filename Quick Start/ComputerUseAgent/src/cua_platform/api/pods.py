...
from cua_platform.api.novnc import create_proxy_session
from cua_platform.pods.models import POD_STATUS_OCCUPIED, POD_STATUS_RUNNING
...
_STREAMABLE_POD_STATUSES = {POD_STATUS_RUNNING, POD_STATUS_OCCUPIED}


async def _ensure_pod_exists(pool: PodPoolService, repo: PodRepository, config, pod_id: str):
    ...
    existing = repo.get(config.account_id, pod_id)
...
async def _ensure_pod_streamable(pool: PodPoolService, repo: PodRepository, config, pod_id: str) -> None:
    pod = await _ensure_pod_exists(pool, repo, config, pod_id)
    if pod.pod_status_code in _STREAMABLE_POD_STATUSES:
        return
    raise api_error(
        409,
        "pod_unavailable",
        "Pod is not available for streaming",
        {
            "pod_status_code": pod.pod_status_code,
            "status_name": pod.status_name,
        },
    )
...
async def get_pod_detail(
    ...
    try:
        ...
        cached = repo.get_detail(config.account_id, pod_id)
...
async def create_pod_stream_session(
    ...
    await _ensure_pod_streamable(pool, repo, config, pod_id)
    try:
        session = await request.app.state.stream_token_gateway.create_session(
            config,
            ecsid=pod_id,
        )
    except UniversalRemoteError as exc:
    ...
    proxy_sid = create_proxy_session(
        request.app.state.novnc_proxy_store,
        viewer_url=session.get("HttpViewerUrl") or session.get("ViewerUrl"),
        websocket_url=session.get("WsWebsocketUrl") or session.get("WebsocketUrl"),
        expires_at=session.get("ExpiresAt"),
    )
    return {
        "account_id": config.account_id,
        "ecsid": pod_id,
        "proxy_viewer_url": f"/novnc/view?sid={proxy_sid}" if proxy_sid else None,
        "viewer_url": session.get("ViewerUrl"),
        "websocket_url": session.get("WebsocketUrl"),
        "http_viewer_url": session.get("HttpViewerUrl"),
        "ws_websocket_url": session.get("WsWebsocketUrl"),
        "expires_at": session.get("ExpiresAt"),
    }
