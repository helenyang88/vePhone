...
describe("authentication gate", () => {
  ...
  it("redirects anonymous initialized users to login", async () => {
    ...
    expect(screen.getByRole("heading", { name: /让电脑桌面自动化测试/ })).toBeVisible();
    expect(screen.getByText("让电脑桌面自动化测试")).toHaveClass("auth-title-primary");
    expect(screen.getAllByText("CUA Test")).toHaveLength(2);
    expect(screen.getByText("Computer Use automation")).toBeVisible();
    expect(screen.getByText("登录你的 CUA Test 账号")).toBeVisible();
    expect(screen.getByText("用例库与测试计划统一维护")).toBeVisible();
    expect(screen.getByText("Computer Use 桌面执行链路")).toBeVisible();
    ...
    expect(screen.queryByText("Mobile UI automation")).not.toBeInTheDocument();
    expect(screen.queryByText("Mobile Use 真机执行链路")).not.toBeInTheDocument();
