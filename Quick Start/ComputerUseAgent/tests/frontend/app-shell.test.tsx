...
it("orders primary navigation and hides the new-task entry", () => {
  ...
  expect(screen.getByText("CUA")).toHaveClass("sidebar-brand-name");
...
it("creates a business from a modal and writes its default execution config", async () => {
  ...
  await userEventApi.type(within(dialog).getByLabelText("CUA AccountId"), "2100000000000000000");
...
it("does not create a business when default execution config is incomplete", async () => {
  ...
  expect(within(dialog).getByText("默认执行配置必须完整：CUA AccountId、AK/SK 都必填。")).toBeVisible();
